...
#         """
#         based on action to update node_id(cui)
#         """
#         if action == 0:
#             # 1-hop search: 移動到最近的鄰居
#             neighbors = self.find_one_hop_neighbors(agent.node_id)
#             if neighbors:
#                 agent.node_id = neighbors[0]['neighbor_cui']

#         elif action == 1:
#             # 跨群 search: 隨機選擇一個新群體中的節點
#             ## TODO: 跨群search 
#             pass
#             # neighbors = self.find_one_hop_neighbors(agent.node_id)
#             # cross_group_neighbors = [n for n in neighbors if n['neighbor_cui'] not in agent.visited_nodes]
#             # if cross_group_neighbors:
#             #     agent.node_id = cross_group_neighbors[0]['neighbor_cui']

#         elif action == 2:
#             # 停止 search，不變更 node_id
#             pass
...
# print("Initial Observation:", test_env.scenario.observation(test_agent, test_env.world))
